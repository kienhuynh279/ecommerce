...
const Panel: FC<PanelProps> = (props) => {
  ...
  const checkMatchPaths = useCallback(
    (_item: IRoute) => {
  ...
  return (
    <div>
      <ButtonBase
        className={clsx('admin-sidebar-item w-full flex justify-between has-submenu', {
          open: !collapsed,
        })}
        onClick={handleClick}
      >
        <div className="flex items-center sidebar-navbar-left">
          <Icon item={item} />
          <span className="align-middle admin-sidebar-navbar-name">{item.name ? t(item.name) : ''}</span>
        </div>

        {Boolean(item.badge) && (
          <div className={`rounded px-1 py-1px bg-${item.badge?.color}`}>{item.badge?.value}</div>
        )}
